...
async function getTramiteCountPerMonth(year) {
    try {
        const token = getToken();
        let response = await fetch(`https://localhost:7285/api/Tramites/GetTramiteCountPerMonth/${year}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        const tramiteCounts = await response.json();
                ...
                scales: {
                    ...
                    yAxes: [{
                        ticks: {
                            maxTicksLimit: 5,
                            padding: 10,
                            // Mostrar el valor de la cantidad
